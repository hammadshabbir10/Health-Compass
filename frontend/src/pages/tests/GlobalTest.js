      ...
      <main style={{ maxWidth: '980px', margin: '0 auto', padding: '2.5rem 2rem 4rem' }}>
        ...
        >
          <div style={{ display: 'grid', gap: '1rem', alignItems: 'center', gridTemplateColumns: '1.4fr 1fr' }}>
            <div>
              <h2 style={{ margin: 0, fontSize: '2rem' }}>{test?.title || 'Global Cognitive Screen'}</h2>
              <p style={{ marginTop: '0.6rem', color: '#cbd5f5', lineHeight: 1.7 }}>
                This section provides a standardized overview of cognitive function, similar to MoCA/MMSE style
                screening, to anchor the overall profile.
              </p>
            </div>
            <div
              style={{
        ...
        >
          <h3 style={{ marginTop: 0 }}>Why These Domains Matter</h3>
          <div style={{ display: 'grid', gap: '0.75rem' }}>
            {[
              {
                domain: 'Global',
                testType: 'MoCA / MMSE',
                benefit: 'Provides a standardized baseline rank for the patient.',
              },
              {
                domain: 'Episodic Memory',
                testType: 'Delayed Recall',
                benefit: 'Separates age-related forgetfulness from pathological loss.',
              },
              {
                domain: 'Executive',
                testType: 'Trail Making Test B',
                benefit: 'Captures the breakdown of complex thinking.',
              },
              {
                domain: 'Language',
                testType: 'Category Fluency',
                benefit: 'Detects word-finding difficulties (aphasia).',
              },
              {
                domain: 'Functional',
                testType: 'IADL Scale',
                benefit: 'Connects cognitive scores to real-world impact.',
              },
            ].map((row) => (
              <div
                key={row.domain}
                style={{
                  display: 'grid',
                  gridTemplateColumns: 'minmax(120px, 1fr) minmax(160px, 1fr) minmax(200px, 2fr)',
                  gap: '1rem',
                  padding: '0.75rem 0',
              ...
              >
                ...
                <div style={{ color: '#cbd5f5' }}>{row.testType}</div>
                <div style={{ color: '#cbd5f5' }}>{row.benefit}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {!test && (
          ...
          >
            <div style={{ fontWeight: 600, fontSize: '1.05rem' }}>Global test is not ready yet.</div>
